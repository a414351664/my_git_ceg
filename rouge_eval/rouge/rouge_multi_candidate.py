...
class Rouge:
    ...
    def _get_avg_scores(self, hyps, refs):
        ...
        for (hyp, ref) in zip(hyps, refs):
            hyp = [" ".join(_.split()) for _ in hyp.split(".") if len(_) > 0]
            ref = [" ".join(_.split()) for _ in ref.split("\t") if len(_) > 0]
            # consider multiple sens
            fn = Rouge.AVAILABLE_METRICS['rouge-l']
            scores_max = []
            for i in ref:
                sc = fn(hyp, [i], exclusive=self.exclusive)
                scores_max.append(int(sc['f']*100))
            index = scores_max.index(max(scores_max))
            ref = [ref[index]]
